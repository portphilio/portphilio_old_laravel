...
@endsection
@section('page-header', 'Edit '.$artifact->title)
@section('main-content')
    <div class="row">
        <div class="col-xs-12 col-sm-12 ">
            <div class="well">
                <h4 class="blue">Update Artifact Source</h4>
            ...
            <h3 class="blue">Update Artifact Details</h3>
            <form id="edit-artifact-form" class="form-horizontal" action="/artifacts/{{ $artifact->id }}" method="post">
                {!! csrf_field() !!}
                <input type="hidden" name="_method" value="put">
                <input type="hidden" name="user_id" id="user_id" value="{{ $artifact->user->id }}">
                <input type="hidden" name="thumbnail" id="thumbnail" value="{{ $artifact->thumbnail }}">
                <input type="hidden" name="type" id="type" value="{{ $artifact->type }}">
                <div class="form-group">
                    <label class="col-sm-2 control-label no-padding-right" for="url">Link</label>
                    <div class="col-sm-10">
                        <input type="text" id="url" name="url" placeholder="Set Automatically" class="col-xs-10 col-sm-5" value="{{ $artifact->url }}">
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-sm-2 control-label no-padding-right" for="title">Title</label>
                    <div class="col-sm-10">
                        <input type="text" id="title" name="title" placeholder="Artifact Title" class="col-xs-10 col-sm-5" value="{{ $artifact->title }}">
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        <div class="input-group col-xs-10 col-sm-5">
                            <input type="text" id="from" name="from" class="form-control date-picker" data-date-format="yyyy-mm-dd" value="{{ $artifact->from }}"><span class="input-group-addon"><i class="fa fa-calendar bigger-110"></i></span>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        ...
                        <div class="input-group">
                            <input type="text" id="to" name="to" class="form-control date-picker" data-date-format="yyyy-mm-dd" value="{{ $artifact->to }}"><span class="input-group-addon"><i class="fa fa-calendar bigger-110"></i></span>
                        </div>
                        </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        <input type="hidden" id="description" name="description" value="{{ $artifact->description }}">
                        <div id="description-editor" class="wysiwyg-editor">{{ $artifact->description }}</div>
                    </div>
                </div>
                <div class="form-group">
    ...
    <script>
        ...
        (function($){
            var github_token = "{{ $user->getLinkByProvider('github')->oauth2_access_token }}",
                ...
                ongoing = {{ !empty($artifact->from) && empty($artifact->to) ? 'true' : 'false' }};
            ...
            $('#edit-artifact-form').on('submit', function() {
